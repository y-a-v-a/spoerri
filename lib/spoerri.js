...
function createTableImage() {
  ...
  const nTableGuests = Math.round(Math.random() * 14) + 2;
  ...
  Promise.all(pImageUrls).then((aImageUrls) => {
    ...
    Promise.all(pImagesFetched).then((aImages) => {
      debug('%d images fetched', aImages.length);
      ...
      aImages.forEach((gdImage, nIndex) => {
        ...
        let x = ((nIndex > (nGridsize - 1) ? nIndex - nGridsize : nIndex) * 250);
      ...
      const newFileName = `./tmp/result${Date.now()}.jpg`;

      gdTargetImage.saveJpeg(newFileName, Math.round(Math.random() * 100), (error) => {
        if (error) {
          debug('GD error: %s while saving %s', error, newFileName);
        }
      ...
      debug('Error image fetch: %s', message);
    });
  }).catch((message) => {
    debug('Error URLs fetch: %s', message);
  });
}
