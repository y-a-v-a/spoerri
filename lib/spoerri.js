  ...
  Promise.all(pImageUrls).then((aImageUrls) => {
    ...
    Promise.all(pImagesFetched).then((aImages) => {
      ...
      aImages.forEach((gdImage, nIndex) => {
        ...
        let offset = (gdImage.width > 10 && gdImage.height) > 10 ? 10 : 0;
        const nColor = gdImage.getPixel(offset, offset);
      ...
      const newFileName = `./tmp/result-${uuid}.jpg`;

      gdTargetImage.saveJpeg(newFileName, Math.round(Math.random() * 100), (error) => {
        if (error) {
          debug(`GD error for ${uuid}: %s while saving %s`, error, newFileName);
        }
